...
### 修复：iOS 七牛云同步拉取数据失败（重点）

- **下载改为分片 Range 下载（原生平台）**：七牛下载域名支持 `Range` 分段请求，
  每次只传输 64KB（base64 后约 85KB），彻底规避超大 base64 响应问题；
  分片失败自动回退一次性下载。
- **读取失败不再静默**：`read()` 失败会抛出具体错误（区分"云端无文件"与
  "读取失败"），设置页同步区域会显示详细错误原因，不再出现
  "显示同步正常却拉不到数据"的假象。
- **防覆盖保护（数据安全）**：云端读取失败时**中止本次同步**，绝不用本地空数据
  覆盖云端，避免云端任务被误清空。
- 同步链路增加详细日志，便于定位问题。

### 其他

- 七牛同步链路全流程验证：下载 572KB 数据文件、分片拼接与云端 md5 一致。
