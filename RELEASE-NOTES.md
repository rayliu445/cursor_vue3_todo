...
### 修复：同步"假正常"——数据拉不到但显示同步正常

- **根因**：启动时自动同步（`initialSync`）的失败被 `catch` 静默吞掉，
  只打印日志不上报状态 → 云端读取失败时设置页仍显示"同步正常"，
  但数据根本没拉进来（v0.1.9 之前 + 启动自动同步时均如此）。
- **修复**：`initialSync` 失败不再吞错误——明确设置"同步出错"状态并在
  设置页显示具体错误（`初始同步失败：xxx`）。
- **新增「同步诊断」面板**（设置 → 同步）：显示最近一次同步的真相——
  云端文件大小、云端任务数、本地任务数、合并后任务数、失败阶段与错误。
  无论成功失败都能看到具体数字，彻底告别"同步正常却不知道发生了什么"。
- `syncNow`（手动同步）也记录诊断详情。
